...
Quand l'upload invités est activé **et** que « Inclure dans la galerie officielle » est coché dans `/admin/guest-uploads`, la galerie (`/gallery`) affiche aussi les photos invités approuvées, avec un filtre supplémentaire **Toutes / Officielles / Invités**. Les photos invités y sont signalées par un badge « Invité » (coin de la vignette, ne recouvre pas l'image). Quand les votes sont activés (`/admin/votes`), elles sont votables au même titre que les captures officielles — chaque source a son propre score, sans interférence entre les deux même en cas d'identifiants numériques identiques. Ce comportement est entièrement désactivé par défaut : la galerie reste inchangée tant que ces réglages ne sont pas explicitement activés. Le badge « Invité » apparaît aussi sur le diaporama `/bestof` pour les photos invités qui y sont diffusées.

## Tags & ID média
Depuis `/admin/tags` (activable, désactivé par défaut) : un bouton « Tags » apparaît sur l'écran de relecture du kiosque (photo, vidéo, photo strip) et ouvre un modal permettant d'associer des tags à la capture qui vient d'être prise — des tags prédéfinis (liste gérée dans l'admin : ajout/édition/suppression, libellé + ordre) et/ou un tag « libre » saisi via un clavier virtuel AZERTY tactile intégré (aucun clavier matériel requis sur le kiosque), avec des règles réglables depuis l'admin (longueur min/max, nombre max de tags par média). Chaque capture officielle (et chaque upload invité) reçoit en plus un identifiant numérique unique (« ID média », longueur réglable dans `/admin/tags`, 3 à 12 chiffres), généré automatiquement, recherchable depuis un champ dédié dans la galerie (`/gallery?q=...`) et affichable en overlay sur le diaporama `/bestof` (option « Afficher l'ID média »).
